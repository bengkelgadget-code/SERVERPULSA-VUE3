        ...
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="filteredProducts.length === 0">
              <td colspan="7" class="px-4 py-6 text-center text-gray-400">Belum ada data PERDANA</td>
            </tr>
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="p-6 space-y-4">
          <div>
            ...
            <div class="flex items-center gap-2">
              <select v-model="form.provider_kategori" class="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none text-sm transition-all">
                <option value="">-- Pilih Provider --</option>
                <option v-for="cat in categories" :key="cat.id" :value="cat.nama">{{ cat.nama }}</option>
              </select>
              <button type="button" @click="openCategoryModal()" class="px-3 py-2 bg-emerald-100 text-emerald-600 hover:bg-emerald-200 rounded-xl transition-colors shadow-sm flex items-center justify-center font-bold">
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="p-4 space-y-4">
          <div class="flex gap-2">
            <input v-model="categoryForm.nama" type="text" class="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none text-sm transition-all" placeholder="Nama Provider baru...">
            <button @click="saveCategory" :disabled="catLoading" class="px-4 py-2 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors shadow-sm disabled:opacity-50">
    ...
    <div v-if="showBatchModal" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-[55] flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-5xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div class="px-5 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between shrink-0">
          <button @click="addBatchRow" class="flex items-center gap-2 text-sm font-semibold text-emerald-600 hover:text-emerald-700 px-3 py-2 rounded-xl hover:bg-emerald-100 transition-colors">
