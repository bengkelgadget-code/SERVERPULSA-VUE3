...
<template>
  <div class="h-full overflow-y-auto max-w-6xl mx-auto p-4 md:p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-2">
        <router-link to="/tv/vouchers" target="_blank" class="bg-indigo-100 hover:bg-indigo-200 text-indigo-700 px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-colors">
        ...
        <button @click="openBatchModal" class="bg-purple-100 hover:bg-purple-200 text-purple-700 px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-colors">
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-5xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 class="font-bold text-lg text-gray-900 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>
            Input Massal Voucher
          </h3>
            ...
            <table class="w-full text-sm text-left">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="(row, idx) in batchForm" :key="idx" class="hover:bg-gray-50/50 group">
                  <td class="p-2">
                    <select v-model="row.provider_kategori" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none text-sm">
                      <option value="">-- Pilih --</option>
                      <option v-for="cat in categories" :key="cat.id" :value="cat.nama">{{ cat.nama }}</option>
                    </select>
                  </td>
                  <td class="p-2">
                    <input v-model="row.nama_produk" type="text" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none text-sm" placeholder="Nama Produk">
                  </td>
                  <td class="p-2">
                    <input :value="formatInputRp(row.harga_beli)" @input="handleBatchRpInput(idx, 'harga_beli', $event)" type="text" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none text-sm text-right">
                  </td>
                  <td class="p-2">
                    <input :value="formatInputRp(row.harga_jual)" @input="handleBatchRpInput(idx, 'harga_jual', $event)" type="text" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none text-sm text-right font-semibold">
                  </td>
                  <td class="p-2">
                    <input :value="formatInputRp(row.stok)" @input="handleBatchRpInput(idx, 'stok', $event)" type="text" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none text-sm text-center">
                  </td>
