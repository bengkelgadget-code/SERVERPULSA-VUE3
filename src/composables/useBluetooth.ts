...
export function useBluetooth() {
  ...
  function formatReceipt(transaction: any, storeName: string): string {
    ...
    text += '--------------------------------' + LF
    text += ALIGN_LEFT
    const dateStr = transaction.created_at ? new Date(transaction.created_at).toLocaleString('id-ID') : '-';
    text += `Tgl   : ${dateStr}` + LF
    ...
    const total = transaction.price || transaction.amount || 0;
    text += `TOTAL : Rp ${total.toLocaleString('id-ID')}` + LF
    text += BOLD_OFF + LF
